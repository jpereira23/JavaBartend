 ...
 */
public class BartendEmitter extends Emitter{
     ...
     */
    public BartendEmitter on(String event, Listener fn) {
      System.out.println("The socket is triggering to be on");
     ...
     * @return a reference to this object.
     */
    public BartendEmitter once(final String event, final Listener fn) {
     ...
     * @return a reference to this object.
     */
    public BartendEmitter off(String event) {
     ...
     * @return a reference to this object.
     */
    public BartendEmitter off(String event, Listener fn) {
        ConcurrentLinkedQueue<Listener> callbacks = this.callbacks.get(event);
        if (callbacks != null) {
            Iterator<Listener> it = callbacks.iterator();
            while (it.hasNext()) {
                Listener internal = it.next();
                if (BartendEmitter.sameAs(fn, internal)) {
                    it.remove();
                    break;
                }
            }
     ...
     * @return a reference to this object.
     */
    public BartendEmitter emit(String event, Object... args) {
        ConcurrentLinkedQueue<Listener> callbacks = this.callbacks.get(event);
        if (callbacks != null) {
    ...
    */
    private class OnceListener implements Listener {
        ...
        @Override
        public void call(Object... args) {
            BartendEmitter.this.off(this.event, this);
            this.fn.call(args);
        }
    }
}
